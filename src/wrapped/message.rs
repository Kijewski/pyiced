...
impl WrappedMessage {
    #[new]
    fn new(py: Python, python: Py<PyAny>) -> PyResult<Self> {
        match python {
            python if !python.is_none(py) => Ok(WrappedMessage(Message::Python(python))),
            _ => Err(PyErr::new::<PyTypeError, _>(
                "Cannot pass None to Message()",
            )),
        }
    }

    /// Whether the message was generated by python code ("python"), or user interaction ("native").
    ///
    /// Returns
    /// -------
    /// str
    ///     Either "native" or "python".
    #[getter]
    fn r#type(&self) -> Option<&'static str> {
        match &self.0 {
            Message::None => None,
            Message::Native(_) => Some("native"),
            Message::Python(_) => Some("python"),
        }
    }
    ...
    #[getter]
    fn python(&self) -> Option<Py<PyAny>> {
        match &self.0 {
